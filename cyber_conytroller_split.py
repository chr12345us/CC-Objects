...
def split_devices(source_json, output_file1, output_file2):
    ...
    # Save the new files
    success1 = save_json_file(data1, output_file1)
    success2 = save_json_file(data2, output_file2)
    
    # Print summary
    print(f"\nSummary:")
    print(f"Total devices in source file: {len(data.get('devices', []))}")
    print(f"Devices in {output_file1}: {len(data1['devices'])} (dp01 and dp03)")
    print(f"Devices in {output_file2}: {len(data2['devices'])} (dp02 and dp04)")
...
def parse_arguments():
    ...
    parser.add_argument('-o1', '--output1', default='dp01_dp03_devices.json', 
                        help='Output file for dp01 and dp03 devices (default: dp01_dp03_devices.json)')
    parser.add_argument('-o2', '--output2', default='dp02_dp04_devices.json', 
                        help='Output file for dp02 and dp04 devices (default: dp02_dp04_devices.json)')
...
def main():
    ...
    print(f"Output file 1 (dp01/dp03): {args.output1}")
    print(f"Output file 2 (dp02/dp04): {args.output2}")
    
    # Split the devices and create the output files
    split_devices(args.source, args.output1, args.output2)
